# CLAUDE.md — audit-stats

This sub-project inherits all WAT framework instructions from the root `CLAUDE.md`. Only project-specific overrides are documented here.
...
- **Model:** `claude-opus-4-6` (set via `AI_MODEL` in `.env`)
- **Provider:** `claude` (set via `AI_PROVIDER` in `.env`)
